...
const MOBILE_STICKY_TOP = 68
const DESKTOP_ENTER_OFFSET_VH = 62
const TABLET_ENTER_OFFSET_VH = 54
const MOBILE_ENTER_OFFSET_VH = 42
...
export function ServicesStackedCardsScroll({
  ...
  const enterOffsetPx =
    ((isMobile ? MOBILE_ENTER_OFFSET_VH : isTablet ? TABLET_ENTER_OFFSET_VH : DESKTOP_ENTER_OFFSET_VH) /
      100) *
    viewport.height
  const segmentSize = 1 / Math.max(cards.length - 1, 1)
  const segmentMotionRatio = isMobile ? 0.92 : isTablet ? 0.88 : 0.82
        ...
        <div ref={stageRef} className={styles.stickyStage}>
          <div className={styles.stageFrame}>
            {cards.map((card, index) => {
              ...
              const segmentStart = segmentSize * (index - 1)
              const segmentEnd = segmentStart + segmentSize * segmentMotionRatio
